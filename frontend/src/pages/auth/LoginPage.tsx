...
const LoginPage: React.FC = () => {
  ...
  const [biometricMethods, setBiometricMethods] = useState<string[]>([]);
  ...
  const checkBiometricAvailability = async () => {
    try {
      const hasBiometric = await biometricService.hasAnyBiometricAvailable();
      ...
      if (hasBiometric) {
        ...
        const availableMethods = Object.entries(status)
          .filter(([_, methodStatus]) => methodStatus.isAvailable)
          .map(([method, _]) => method);
        setBiometricMethods(availableMethods);
      }
    } catch (error) {
  ...
  const handleBiometricSuccess = (userData: any) => {
    toast.success(`Biometric authentication successful! Welcome back, ${userData.userId}`);
        ...
        <Card className="shadow-lg border-0">
          ...
          <CardContent>
            <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
              ...
              <TabsContent value="biometric" className="space-y-0">
                {biometricAvailable ? (
                  renderBiometricLogin()
                ) : (
                  <div className="text-center py-8">
                    <LockClosedIcon className="mx-auto h-12 w-12 text-gray-400" />
                    <h3 className="mt-2 text-sm font-medium text-gray-900">Biometric Not Available</h3>
                    <p className="mt-1 text-sm text-gray-500">
                      Biometric authentication is not available on this device.
                    </p>
                    <div className="mt-6">
                      <Button onClick={() => setActiveTab('password')} variant="outline">
        ...
        {biometricAvailable && (
          <div className="text-center">
            <p className="text-xs text-gray-500">
              Available methods: {biometricMethods.join(', ')}
            </p>
          </div>
        )}
      </div>
