...
public class ServerRuntimeLogger {
    ...
    public void initialize() {
        loggingService.info("ServerRuntimeLogger", "Server runtime monitoring initialized");
    ...
    private void logSystemInfo() {
        try {
            ...
            loggingService.info("SystemInfo", "Server system information");
            
        } catch (Exception e) {
            loggingService.error("SystemInfo", "Failed to log system information");
        }
    }
    ...
    public void monitorSystemHealth() {
        try {
            ...
            loggingService.error("HealthMonitor", "Error during health monitoring");
        }
    }
    ...
    public void logDetailedMetrics() {
        try {
            ...
            loggingService.error("MetricsLogger", "Error logging detailed metrics");
        }
    }
    ...
    private void monitorMemory() {
        try {
            ...
            if (heapUsagePercent > 90) {
                loggingService.critical("MemoryMonitor", "Critical memory usage detected", 
                    Map.of("heapUsagePercent", heapUsagePercent, "heapUsed", heapUsed, "heapMax", heapMax),
                    null, null, null, null, null);
            } else if (heapUsagePercent > 80) {
                loggingService.warn("MemoryMonitor", "High memory usage detected", 
                    Map.of("heapUsagePercent", heapUsagePercent, "heapUsed", heapUsed, "heapMax", heapMax),
                    null, null, null, null, null);
            }
            ...
            loggingService.debug("MemoryMonitor", "Memory metrics collected");
            
        } catch (Exception e) {
            loggingService.error("MemoryMonitor", "Error monitoring memory");
        }
    }
    ...
    private void monitorThreads() {
        try {
            ...
            if (threadCount > 1000) {
                loggingService.warn("ThreadMonitor", "High thread count detected", 
                    Map.of("threadCount", threadCount, "peakThreadCount", peakThreadCount),
                    null, null, null, null, null);
            }
            ...
            if (deadlockedThreads != null && deadlockedThreads.length > 0) {
                loggingService.critical("ThreadMonitor", "Deadlocked threads detected", 
                    Map.of("deadlockedThreadCount", deadlockedThreads.length, "deadlockedThreadIds", deadlockedThreads),
                    null, null, null, null, null);
            }
            ...
            loggingService.debug("ThreadMonitor", "Thread metrics collected");
            
        } catch (Exception e) {
            loggingService.error("ThreadMonitor", "Error monitoring threads");
        }
    }
    ...
    private void monitorClassLoading() {
        try {
            ...
            loggingService.debug("ClassMonitor", "Class loading metrics collected");
            
        } catch (Exception e) {
            loggingService.error("ClassMonitor", "Error monitoring class loading");
        }
    }
    ...
    private void monitorPerformance() {
        try {
            ...
            if (operatingSystemMXBean instanceof com.sun.management.OperatingSystemMXBean) {
                ...
                if (cpuLoad > 0.9) {
                    loggingService.warn("PerformanceMonitor", "High CPU usage detected", 
                        Map.of("cpuLoad", cpuLoad), null, null, null, null, null);
                }
                
                // System load average
                double systemLoadAverage = operatingSystemMXBean.getSystemLoadAverage();
                if (systemLoadAverage > 0.8) {
                    loggingService.warn("PerformanceMonitor", "High system load detected", 
                        Map.of("systemLoadAverage", systemLoadAverage), null, null, null, null, null);
                }
            }
            
        } catch (Exception e) {
            loggingService.error("PerformanceMonitor", "Error monitoring performance");
        }
    }
        ...
        try {
            ...
            if (totalRequestsCount > 0) {
                ...
                if (errorRate > 10) { // More than 10% error rate
                    loggingService.critical("ErrorMonitor", "Critical error rate detected", 
                        Map.of("errorRate", errorRate, "totalRequests", totalRequestsCount, "totalErrors", totalErrorsCount),
                        null, null, null, null, null);
                } else if (errorRate > 5) { // More than 5% error rate
                    loggingService.warn("ErrorMonitor", "High error rate detected", 
                        Map.of("errorRate", errorRate, "totalRequests", totalRequestsCount, "totalErrors", totalErrorsCount),
                        null, null, null, null, null);
                }
                
                if (warningRate > 20) { // More than 20% warning rate
                    loggingService.warn("ErrorMonitor", "High warning rate detected", 
                        Map.of("warningRate", warningRate, "totalRequests", totalRequestsCount, "totalWarnings", totalWarningsCount),
                        null, null, null, null, null);
                }
            }
            
        } catch (Exception e) {
            loggingService.error("ErrorMonitor", "Error monitoring error rates");
        }
    }
    ...
    private void logDetailedSystemMetrics() {
        try {
            ...
            loggingService.info("DetailedMetrics", "Detailed system metrics logged");
            
        } catch (Exception e) {
            loggingService.error("DetailedMetrics", "Error logging detailed metrics");
        }
    }
    ...
    public void logRuntimeError(String category, String message, Exception exception, 
        ...
        loggingService.logException(category, message, exception, userId, sessionId, requestId, ipAddress, userAgent);
    }
    
    public void logRuntimeWarning(String category, String message, Map<String, Object> details,
                                 String userId, String sessionId, String requestId, String ipAddress, String userAgent) {
        incrementWarningCount();
        loggingService.warn(category, message, details, userId, sessionId, requestId, ipAddress, userAgent);
    }
    ...
    public boolean isSystemHealthy() {
        try {
            ...
            loggingService.error("HealthCheck", "Error during health check");
            return false;
        }
    }
    ...
    public Map<String, Object> getSystemHealthReport() {
        try {
            ...
            loggingService.error("HealthReport", "Error generating health report");
